...
if (file.exists("1_NF-Kappa_B.txt.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("1_NF-Kappa_B.txt.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("1_NF-Kappa_B.txt", header=T, com='', row.names=1, check.names=F, sep='\t')
...
write.table(data, file="1_NF-Kappa_B.txt.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
heatmap_data = data
pdf("1_NF-Kappa_B.txt.log2.centered.genes_vs_samples_heatmap.pdf")
...
", "1_NF-Kappa_B.txt.log2.centered" ) )
dev.off()
