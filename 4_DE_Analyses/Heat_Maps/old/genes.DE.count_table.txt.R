...
if (file.exists("genes.DE.count_table.txt.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("genes.DE.count_table.txt.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("genes.DE.count_table.txt", header=T, com='', row.names=1, check.names=F, sep='\t')
...
write.table(data, file="genes.DE.count_table.txt.minRow10.CPM.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
write.table(sample_cor, file="genes.DE.count_table.txt.minRow10.CPM.log2.centered.sample_cor.dat", quote=F, sep='	')
sample_dist = dist(t(data), method='euclidean')
hc_samples = hclust(sample_dist, method='complete')
pdf("genes.DE.count_table.txt.minRow10.CPM.log2.centered.sample_cor_matrix.pdf")
sample_cor_for_plot = sample_cor
if (is.null(hc_samples)) { RowV=NULL; ColV=NULL} else { RowV=as.dendrogram(hc_samples); ColV=RowV }
heatmap.3(sample_cor_for_plot, dendrogram='both', Rowv=RowV, Colv=ColV, col = myheatcol, scale='none', symm=TRUE, key=TRUE,density.info='none', trace='none', symkey=FALSE, symbreaks=F, margins=c(10,10), cexCol=0.5, cexRow=0.5, cex.main=0.75, main=paste("sample correlation matrix
", "genes.DE.count_table.txt.minRow10.CPM.log2.centered") , ColSideColors=sampleAnnotations, RowSideColors=t(sampleAnnotations))
dev.off()
pdf("genes.DE.count_table.txt.minRow10.CPM.log2.centered.prcomp.principal_components.pdf")
prin_comp_data = data
...
write.table(pca$rotation, file="genes.DE.count_table.txt.minRow10.CPM.log2.centered.PCA.prcomp.scores", quote=F, sep="	")
write.table(pca$x, file="genes.DE.count_table.txt.minRow10.CPM.log2.centered.PCA.prcomp.loadings", quote=F, sep="	")
PCA.loadings=pca$x
...
heatmap_data = data
pdf("genes.DE.count_table.txt.minRow10.CPM.log2.centered.genes_vs_samples_heatmap.pdf")
