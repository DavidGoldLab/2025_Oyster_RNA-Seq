...
if (file.exists("1_Mucins.txt.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("1_Mucins.txt.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("1_Mucins.txt", header=T, com='', row.names=1, check.names=F, sep='\t')
...
data = zscaled_data
data = na.omit(data)
write.table(data, file="1_Mucins.txt.log2.ZscaleRows.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
heatmap_data = data
pdf("1_Mucins.txt.log2.ZscaleRows.genes_vs_samples_heatmap.pdf")
