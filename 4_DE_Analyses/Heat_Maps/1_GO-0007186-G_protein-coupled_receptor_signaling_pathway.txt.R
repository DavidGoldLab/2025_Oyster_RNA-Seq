...
if (file.exists("1_GO-0007186-G_protein-coupled_receptor_signaling_pathway.txt.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("1_GO-0007186-G_protein-coupled_receptor_signaling_pathway.txt.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("1_GO-0007186-G_protein-coupled_receptor_signaling_pathway.txt", header=T, com='', row.names=1, check.names=F, sep='\t')
...
data = zscaled_data
data = na.omit(data)
write.table(data, file="1_GO-0007186-G_protein-coupled_receptor_signaling_pathway.txt.log2.ZscaleRows.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
heatmap_data = data
pdf("1_GO-0007186-G_protein-coupled_receptor_signaling_pathway.txt.log2.ZscaleRows.genes_vs_samples_heatmap.pdf")
...
", "1_GO-0007186-G_protein-coupled_receptor_signaling_pathway.txt.log2.ZscaleRows" ) )
dev.off()
