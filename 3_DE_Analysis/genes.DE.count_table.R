...
if (file.exists("genes.DE.count_table.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("genes.DE.count_table.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("genes.DE.count_table", header=T, com='', row.names=1, check.names=F, sep='\t')
...
write.table(data, file="genes.DE.count_table.minRow10.CPM.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
write.table(sample_cor, file="genes.DE.count_table.minRow10.CPM.log2.centered.sample_cor.dat", quote=F, sep='	')
sample_dist = dist(t(data), method='euclidean')
hc_samples = hclust(sample_dist, method='complete')
pdf("genes.DE.count_table.minRow10.CPM.log2.centered.sample_cor_matrix.pdf")
...
", "genes.DE.count_table.minRow10.CPM.log2.centered") , ColSideColors=sampleAnnotations, RowSideColors=t(sampleAnnotations))
dev.off()
pdf("genes.DE.count_table.minRow10.CPM.log2.centered.prcomp.principal_components.pdf")
prin_comp_data = data
...
write.table(pca$rotation, file="genes.DE.count_table.minRow10.CPM.log2.centered.PCA.prcomp.scores", quote=F, sep="	")
write.table(pca$x, file="genes.DE.count_table.minRow10.CPM.log2.centered.PCA.prcomp.loadings", quote=F, sep="	")
PCA.loadings=pca$x
